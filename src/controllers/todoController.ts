...
export const updateTodo = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const currentTodo = await TodoModel.getById(Number(id), userId) as TodoRow | undefined;
    if (!currentTodo) {
      sendError(res, 'Tugas tidak ditemukan!', 404);
      return;
    }
    ...
    const affectedRows = await TodoModel.update(
      Number(id),
      payload.task ?? currentTodo.task,
      payload.is_completed ?? Boolean(currentTodo.is_completed),
      userId
    );
